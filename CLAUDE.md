...
### 🎯 PRÓXIMA SESIÓN — PENDIENTE (actualizado 2026-06-11)

> **Lo que queda, en orden sugerido para arrancar:**
>
> 1. **B.5-7 — UX restante del push/pull (rápidos, ~una tarde).** Inferencia de distancia sobre caras/planos, marcador verde del punto inferido, mensaje "Offset limited to X m" en la status bar. Detalle en el bloque **B** abajo. *Cierran la paridad SketchUp del push/pull.*
> 2. **Los 57 seeds de `KNOWN_BAD`** (fuzz, `tests/test_fuzz_engine.py`) — secuencias multi-step profundas (3-7 ops) que el motor aún no sobrevive, congeladas como `xfail` por seed. Método: `run_sequence(escenario, seed)` para repro → minimizar → regresión nombrada → fix de raíz → `python -m tests.test_fuzz_engine` para regenerar la lista. Ya **no hay repros cortos**; cada seed cuesta entender 4+ pasos. Ver "🧪 Fuzz bench" abajo para el estado y las clases.
> 3. **Bloque C** (features de producto, NO antes de B): Fase 2 selección (doble/triple-click), Fase 3 Eraser, face culling, Groups v2, Fase 4 (Circle/Arc/Tape), Fase 6 (UI capas), **Fase 7 Materials** (ya tiene su base con A.3 hecho) + Dimensions + import/export. Lista completa en el bloque **C** abajo.
>
