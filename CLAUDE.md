...
| **🟡 G1** Teselas satelital (XYZ) *(código hecho 2026-07-09, falta confirmar render en pantalla)* | Imagen base tipo Earth, provider-agnóstica | `georef/tiles.py` (slippy z/x/y + `TileSource` presets Esri/Sentinel-2/OSM + **XYZ custom** + `TileCache` LRU disco + `TileLayer`); `georef/tile_fetcher.py` (`TileFetcher` async `QNetworkAccessManager`, cache-first, coalesce, `default_cache_dir` vía `QStandardPaths`); `core/scene.py` (`+tile_layer`); render pass en `viewport._render_tiles` (quads Z=0 texturizados antes del grid, depth-write off, tex GL cacheada por tesela); UI Tray `BaseMapPanel` (fuente + ir a lat/lon → ancla datum, zoom, toggle, atribución) | Qt (sin pip) | ✅ headless (go-to, cobertura, cambio de fuente) + fetch Esri en vivo; ⚠️ **render en pantalla sin verificar** (QOpenGLWidget no corre offscreen — pedirle al usuario `python main.py` → Bandeja ▸ Mapa base ▸ Ir). 24 tests, 604 verdes |
