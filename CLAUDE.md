...
## 📦 Estado actual (2026-09-04, tarde) — v0.3.11 released: la release de Windows y del intercambio (texturas apiladas, .skp que SketchUp rechazaba, GPU dedicada, MCP empaquetado)

**v0.3.11 released 2026-09-04** (tag `v0.3.11`). Seis arreglos, todos nacidos de que un amigo de Marco (Windows, laptop Intel + RTX 4060) probara la 0.3.10: (1) `3254b93` cada guardado del `.igz` apilaba otro prefijo de hash al nombre de la textura (`textures/<hash>-<hash>-…-sumari.png`) hasta superar los 260 chars de Windows y el archivo no abría → `core.texture.texture_file_name()` limpia y recorta, los archivos hinchados sanan al reguardar; (2) `2837268` el MISMO nombre apilado hacía que el exportador `.skp` pasara una ruta de 300 chars a openskp, cuyo `_write_str` lanza con la imagen ya en el buffer, y el «fallback a color» escribía encima → SketchUp rechazaba el archivo (SUResult 12); ahora `_stage_texture` copia cada textura a un tempdir con nombre corto, la ruta de casa ya no viaja en el `.skp`, y (`8326b5d`) BMP/TIFF/GIF se reconvierten a PNG. **Validar «abre en SketchUp» con `wine ~/.local/share/skp2dae/skp2dae.exe a.skp b.dae`** — que openskp lea su propia salida no basta; (3) `5c039f4` `core.glinfo` anota GL_VENDOR/RENDERER/VERSION en `ingetrazo-gl.txt`, lo muestra en Acerca de y avisa si es rasterizador por software; (4) `5fb7792` `core.gpu_prefs` + instalador escriben `HKCU\Software\Microsoft\DirectX\UserGpuPreferences` = «Alto rendimiento» para `ingetrazo.exe` (laptops híbridas arrancaban en la Intel); (5) `9bb6d37` el paquete no llevaba `scripts/ingetrazo_mcp.py` y la guía decía `python3`: ahora `ingetrazo-mcp.exe` (segundo EXE consola en el spec), `ingetrazo --mcp` en Linux, y diálogo con las líneas exactas al encender el puente. Repro del bug de openskp para Ahsan en `../para-ahsan/openskp-repro-long-path/`. Pendiente manual de Marco: `skp2dae.exe`, snap en PUBLIC, secret `CLOUDFLARE_API_TOKEN`; pendiente de verificar en Windows real: que Claude Desktop arranque `ingetrazo-mcp.exe`.

**+ 04-09 noche — bug «texturas se ven mal en SketchUp» CAZADO Y CORREGIDO (sin publicar).** Oráculo: exportar caras de control, convertir con `skp2dae.exe` bajo Wine y leer los TEXCOORD del DAE (los UV que calcula el SDK real); `tests/test_skp_sdk_oracle.py` lo deja como test (se salta sin Wine) y `test_textures_read_back_where_the_viewport_drew_them` hace lo mismo en CI con el lector calibrado de openskp. Tres causas: (1) `openskp.create._face_uv_basis` resuelve la matriz de `front_uv` en la base «primera arista» y SketchUp la lee en «Z × n» → cada cara girada el ángulo de su 1ª arista; (2) el escritor no multiplica los pins por el applied size, que SketchUp divide al leer → agua de 2 m 78× grande; (3) la proyección por defecto de IngeTrazo (`plane_axes`, X proyectada) difiere 180° de la de SketchUp en muros +Y/−X. Arreglo: `skp_out._writer_uv_quirks` (sonda de COMPORTAMIENTO: escribe una cara conocida y lee la matriz con el parser) + `_compensate_pins` (UV × applied, punto falso cuya proyección en la base del escritor es la del punto real en la de SketchUp; se apaga sola cuando upstream arregle el escritor); `core.texture.projection_basis`/`projection_axes` = receta única (visor, OBJ/glTF/DAE, pegar, importador). Rama del PR lista en `~/openskp` (`fix/front-uv-sketchup-basis-and-applied-size`, textos en `../para-ahsan/openskp-front-uv-fix/`). **Ronda 2 (capturas de Marco en SketchUp Web):** las caras vistas desde atrás salían lavanda (reverso por defecto de SketchUp) porque IngeTrazo pinta las dos caras y el exportador solo escribía el frente → `_emit_face` escribe `back_material`/`back_uv` (mismo material y pins, o los de `attrs["back"]`), guardado por `_both_sides`. Rocas/tronco más claros y bordes negros en hojas = luz y líneas de perfil de SketchUp Web, medido, no bug. **Ronda 3 («algunas están rotadas»):** comparación cara por cara SDK↔visor sobre la piscina real (`front_check.py` en el scratchpad de la sesión, regenerable) → las caras HORIZONTALES salían 90° giradas: `Z × n` es discontinuo en la vertical y `Face._newell` en float32 dejaba normales (2,9e-6, 0, 1)… hasta 6e-4; el escritor guarda el plano en float64 (exacto) y SketchUp lo lee con tolerancia **|Z × n| < 1e-3** (medida con el SDK, ε de 1e-10 a 1e-2: vertical hasta 1,0000004e-3, producto cruz desde 1,0001e-3). Arreglo: `SKETCHUP_VERTICAL_TOLERANCE = 1e-3` en `projection_basis`, `_newell` en doble precisión, `_compensate_pins` usa `_writer_normal` (el `_plane_from_polygon` del escritor). El oráculo del SDK lleva ahora una cara inclinada 1e-4 y otra a 1500 m del origen. **Ronda 3b:** las caras que miran hacia ABAJO (−Z) salían 180°: SketchUp usa (−X, +Y) para −Z (medido con el SDK, misma tolerancia 1e-3), el lector de openskp asume (X, −Y) → corregido en `projection_basis` (vertical: `(X, Y)` si nz>0, `(−X, Y)` si no); el oráculo del SDK lleva 3 pisos boca abajo. **Ronda 4 (captura 20:48, «los personajes no aparecen»):** el exportador saltaba los billboards → `_billboard_definition` los convierte en componentes face-me (quad canónico para `billboard is True`, malla girada con su normal mayor hacia −Y para `"mesh"`; ancla = pies; `_placement`), con `always_faces_camera=True` cuando `add_component_definition` lo acepta (sonda `_supported`; en el fork ya está: commit `2910833`, byte −9 del gap de 43 de la definición; upstream aún no → figuras quietas mirando a −Y). Peso: piscina.igz (0.3.5) tiene 294k caras porque esa importación explotó los componentes; el original de SketchUp Web reimportado hoy da 75k caras y 28,8 MB (SketchUp Web: 14,1 MB, formato comprimido; el nuestro es el legacy 2017 sin comprimir). Texturas en blanco al orbitar en SketchUp Web = su modo de respuesta rápida con modelos pesados, no es del archivo. **Ronda 5 («marco negro alrededor de Sumari»):** el exportador nunca pasaba `hidden_edges` → `_is_hidden_face` (todas las aristas del contorno ocultas) lo activa, y el quad de `_billboard_definition` nace con aristas ocultas; las figuras de malla copian soft/hidden de sus aristas originales. `v8` en Descargas (con el fork). **Ronda 6 (peso):** `_share_repeats` en skp_out: piezas conexas (`_pieces`, union-find por aristas) con huella invariante a posición y giro sobre Z (`_Piece.key`: nº caras/vértices, distancias radiales y alturas redondeadas a 1e-2), alineación por el punto más lejano del eje (`_turn_onto`, verificada punto por punto con `_MATCH_TOL` 2 mm) y pintura cara por cara (`_same_paint`, UV incluidos); copias → definición `Repetido N` + `_Copy(xform)` colocadas por `_placement`; espejos y escalas NO se comparten. piscina.igz: 70,6 → 26,6 MB (66k caras escritas de 294k; 242 definiciones, 7266 instancias; SDK OK; texturas idénticas en el chequeo). **Ronda 7 («Guardado fallido» en SketchUp Web):** el archivo abría pero no se podía guardar (`SUModelSaveToFile` → SUResult 7, serialización). Cazado con `skp2dae-back.exe` en modo `SKP2DAE_RESAVE=<salida>` (re-guarda con el SDK; ojo: la RUTA de entrada con acentos hace fallar el re-guardado bajo Wine, falso positivo). Causa en openskp: cada `_ArchiveWriter` numera PIDs desde 1 y `to_bytes` solo suma materiales+capas al contador (u32 en `_PID_COUNTER_POS`=1987, tratado como u16) → SketchUp renumera duplicados al cargar y no serializa; repro mínimo: definición de 1 cara + definición de 3. Fork: PIDs en una secuencia global (`_pid_start`, `_next_pid`), contador u32 (`4fb5043`). IngeTrazo: `_fix_pid_counter` (sonda `_pid_start`; parchea el u32 con actual + PIDs repartidos) + figuras registradas al final; con upstream pileta y piscina re-guardan (SUResult 0). **05-09 madrugada — compositor:** (a) selección que sobrevive a `_rebuild_canvas` (recuerda modelos; pegar/duplicar limpian antes); (b) **Perfil de terreno en láminas** HECHO: `PerfilTerreno` en core/composition (lista `comp.perfiles`, `_list`, to/from_dict), `paint_perfil_mm` + `PerfilItem` + herramienta `perfil` (icono `comp_perfil`) + panel índice 12 (`_page_perfil`, `_on_perfil_props`, `_reload_perfil_paths`) + `profile_for`/`_profile_sampler` (levantamiento > DEM, caché por (índice, nodos, spacing, sampler), invalidada en `_on_model_version` y al llegar tiles) + impresión; reusa `georef.profile.sample_profile` y `_nice_ticks` del dock. Tests: `tests/test_composer_perfil.py` (StubSampler, sin red); (c) **trazados en los marcos** (Marco: «no se ve la línea del path en composiciones»): `render_image(overlays=False)` deja fuera el overlay QPainter del visor y el HLR solo conoce aristas, así que `compute_annotations` proyecta SIEMPRE `scene.geo_paths` (drapeados con `vp.drape`) como tuplas `("poly", pts)` y `_paint_annots_mm` las pinta en cian con halo y nodos — mismo camino que cotas/textos, que siguen bajo la bandera `annotations`. Progresivas en planta (Marco: «que coincida con el perfil»): `MarcoVista.km_marks/km_step_m` → `_chainage_marks` (tick + rótulo perpendicular por `point_at_station`, fin del trazado incluido si queda a >0,3 paso) y `chainage_step(length, step)` COMPARTIDO con `paint_perfil_mm` (0 = `_nice_ticks(0, L, 6)`), así el «auto» de los dos coincide por construcción. Tests: `tests/test_composer_geo_paths.py`. **Ronda 8 (purga al guardar en SketchUp Web):** Marco guardó y SketchUp ofreció purgar; comparados con openskp los archivos guardados (2,7 / 11,5 MB) vs los nuestros (7,7 / 26,6): mismas definiciones, instancias, caras, aristas, vértices y texturas; solo se fueron capas vacías (8 de 10 en piscina, «Layer 0» en pileta). El 2× es compresión: zlib de los nuestros da 2,4 / 10,9 MB. Geometría compartible casi agotada (51k de 66k caras escritas son únicas; hojas sin pareja no casan ni espejadas). Arreglo: `_collect_layers(scene, builder, used)` escribe solo capas en uso y nunca `DEFAULT_LAYER`. Herramienta útil: `~/.local/share/skp2dae/skp2dae-back.exe` (fuente parcheado en el scratchpad; `SKP2DAE_BACK=1` vuelca los UV del reverso) → el SDK da al reverso los mismos UV que al frente.

**📋 PLAN DE LA PRÓXIMA SESIÓN (fijado con Marco al cierre del 05-09: «afinar composiciones para que se muestren profesionales»; tres capturas de referencia en `~/Imágenes/Capturas de pantalla/Captura desde 2026-09-04 23-1[68]-*.png`: una lámina A101 estilo LayOut con planta + dos secciones + detalle, y dos plantas amobladas cotadas estilo brasileño 1:50).** Cotejo capturas ↔ compositor: YA SALE la planta renderizada con muros cortados en oscuro (sección horizontal + Top + estilo del modelo; `_draw_section_fill`), las cotas rojas en cm (CotaItem: `color`, `units`, `text_mm`), norte, escala gráfica, cajetín con diseños, leyenda de capas, etiqueta con guía, formas. LO QUE FALTA, por impacto visual / coste, y el orden recomendado:
1. ✅ **HECHO 05-09 — Grosores y relleno de corte en el estilo vectorial.** `core/hlr.py::hlr_drawing()` devuelve `HlrDrawing(segs, world, kinds, loops)`: `kinds` = KIND_EDGE / KIND_PROFILE / KIND_CUT por segmento (perfil = silueta soft o arista dura con FONDO a un lado, sondeado en 2D a un pelo del punto medio del tramo visible: `_surface_at`), `loops` = anillos cerrados del corte (`section_loops` encadena las cuerdas de `clip_to_section(split_cuts=True)`; cadenas abiertas se descartan → superficie abierta = sin relleno, como SketchUp pero sin fuga). Cuerdas colineales se fusionan (`_merge_collinear`, evita las costuras de los remates redondos) y los tramos de longitud 2D ~0 se tiran. `hlr_view` es un envoltorio (sin clases ni rellenos). `MarcoVista`: `pen_cut_mm/pen_profile_mm/pen_edge_mm`, `profiles`, `cut_fill` (solid/hatch/none), `cut_fill_color`, `cut_hatch_mm`; el compositor guarda `hlr_kinds`/`hlr_fills` junto a `hlr_cache` (`_forget_frame` limpia todo), el panel del marco tiene la sección «Plumas del vectorial» (`_on_frame_pens`: cambiar pluma o relleno repinta sin recalcular; perfiles on/off y relleno none↔otro sí recalculan). Relleno par-impar (muro hueco queda hueco), pintado ANTES de las líneas y SIN test de oclusión (el plano es lo más cercano en plantas y secciones miradas de frente). DXF: `save_dxf_layers` → capas `X`, `X-PERFIL`, `X-CORTE`. Tests: `tests/test_hlr_line_classes.py`. Pendiente fino: las aristas del fondo del sólido cortado (bajo el poché) siguen en la lista (coinciden con el corte → invisibles, pero duplican líneas en el DXF).
2. ✅ **HECHO 05-09 — Rótulo de vista profesional.** `MarcoVista.title_style` («layout» = burbuja numerada + título + «ESC. 1:N» sobre línea de base, «bar» = franja vertical a la izquierda con título/subtítulo/escala girados y burbuja al pie, «simple» = la línea histórica), `title_text` (vacío = nombre de la vista), `title_subtitle`, `title_number`, `title_sheet` (mitad inferior de la burbuja), `title_scale`, `title_align`, `title_pos` (below/above), `title_mm`. `views/composer.py`: `view_title_texts()` expande campos (`{escala}` lee SIEMPRE ese marco), `view_title_extent()` → (left, top, bottom) mm que el `FrameItem.boundingRect` y el pintor comparten, `_paint_view_title_mm()`. Panel: sección bajo «Rótulo de vista» con `_on_frame_title` (paint-only, no toca cachés); `_on_frame_props` ya solo olvida cachés si cambió algo que afecta al render (título/borde no). Default `title_style="layout"` también para documentos viejos. Tests: `tests/test_composer_view_title.py`.
3. ✅ **HECHO 05-09 — Cotas en cadena.** Herramienta `cota_cadena` (clic; icono `dimension_chain`): en la vista del lienzo `_chain_click(pos, hit)` lleva el estado (`_chain_pts`, `_chain_sep`, `_chain_cotas`): dos puntos → tercer clic fija `sep` → cada clic siguiente `place_chain_cota` (misma línea: `_chain_sep_for` calcula el sep del tramo nuevo para que su línea pase por la línea de la cadena; si el tramo es ⟂ a la cadena conserva el sep). `finish_chain()` (clic sobre el último punto, Esc, o cambio de herramienta vía `cancel_placement`) apila la total con `place_chain_total` (una fila = `text_mm*2+2.5` más afuera, medida desde la línea de la cadena) si hay ≥2 tramos. `_new_cota()` es la fábrica común (place_tool «cota» también la usa). **Estilo de cota por defecto**: `_remember_cota_style` persiste en QSettings `composer/default_cota_style` (JSON) y `_load_default_cota_style` lo carga al abrir el compositor. Tests `tests/test_composer_cota_cadena.py`.
4. **Ejes de referencia** (burbujas A-B-C / 1-2-3 con línea a trazos por planta Y secciones): mejor como entidad de MODELO (`scene.axes`, líneas con nombre en XY) proyectada en todos los marcos por `compute_annotations`, como los trazados — un dibujo, todas las láminas. Medio.
5. **Globos numerados + tabla de leyenda automática** (01..27 → «LEGENDA LAYOUT»): ítem globo (número, anclable como la etiqueta) y tabla que se llena con los globos de la lámina; renumerar por orden. Medio-bajo.
6. **Etiqueta de ambiente**: nombre + área m² (de la cara de piso bajo el punto anclado, `frame_snap_points` ya da el punto mundo) + altura opcional. Medio.
7. **Marcas de sección y llamadas de detalle.** ✅ **7a HECHO 05-09 — marcas de sección**: `MarcoVista.section_marks` (opt-in) → `ComposerWindow._section_marks()` dentro de `compute_annotations` emite `("secmark", x0, y0, x1, y1, ax, ay, letra, size)` por cada plano cuya normal no sea paralela a `fwd` (traza = `n × fwd` proyectada, recortada a la caja del modelo en papel ±6 mm; flechas = −n en papel; letra = `sp.symbol` o A/B/C por índice); pintor `_paint_section_mark_mm` (raya-punto + remate grueso + triángulo + burbuja). `_on_frame_props` distingue `annot_only` (recalcula solo `annot_cache`) de lo que fuerza re-render. Tests `tests/test_composer_section_marks.py`. ✅ **7b HECHO 05-09 — llamadas de detalle**: `LlamadaItem` (`Composicion.llamadas`; caja x/y/w/h, `shape` rect|circle, `number`, `sheet` con campos, burbuja `bx_mm/by_mm` relativa, `frame_uid`+`follow` como el texto ligado → `_follow_commands` ya recorre texts+llamadas), `paint_llamada_mm` (reusa `_paint_title_bubble`; guía `_llamada_leader` = punto más cercano del contorno → borde de la burbuja, ninguna si la burbuja cae dentro), `LlamadaCanvasItem` (mover/redimensionar base + arrastre propio de la burbuja con undo), herramienta `llamada` (arrastre; `place_tool` liga al marco bajo el centro de la caja), página 14 del panel. Tests `tests/test_composer_llamada.py`.
8. ✅ **HECHO 05-09 — Cotas de nivel.** `NivelItem` en `core/composition.py` (`Composicion.niveles`; `level_m()` = Z del ancla o `z_m` menos `datum_m`; `value_text()` «+0.15»/«-0.30»/«±0.00»; `label()` sustituye `{z}` o lo añade), `paint_nivel_mm`/`nivel_bounds_mm` y `NivelCanvasItem` (hereda de `_SheetItem`, NO de EtiquetaCanvasItem: el panel de etiquetas no debe verlo; arrastrar desliza la marca y deja el punto con guía fina, como la etiqueta). Herramienta «nivel» (clic, con snap: `_GEOM_SNAP_TOOLS` ahora incluye `nivel` Y `etiqueta` — la etiqueta nunca anclaba porque no tenía snap), `place_tool` elige círculo si el marco es `std:top`; página 13 del panel (`_page_nivel`/`_on_nivel_props`, recuerda el último estilo en `_last_nivel_style`); reproyección en `_reproject_anchored_cotas` (re-snap del ancla + `x_mm = px - ax_mm`). Icono `comp_nivel`. Tests `tests/test_composer_nivel.py`.
9. **Cajetín vertical** (franja lateral con texto girado, número «03|32»). Bajo-medio.
**+ 05-09 pedido en vivo por Marco: imágenes con opacidad / recorte / desvanecido** — `ImagenItem.opacity/shape(rect|rounded|ellipse)/radius_mm/feather_mm/fit(stretch|cover|contain)/border*`; `views/composer.py::composited_image()` compone en píxeles (NumPy sobre `QImage.bits()`, máscara `_shape_alpha` con distancia con signo + smoothstep para el feather; caché `_IMG_COMPOSITE_CACHE` por cacheKey+campos, tope 2048 px); `paint_image_mm` dibuja la compuesta + contorno `image_outline_path`. Tests `tests/test_composer_image_shape.py`.
Arranque sugerido: 1 → 2 → 8 → 3 (con eso una lámina ya «se ve de oficina»), luego 4/5/6/7 según lo pidan las láminas reales de Marco (pileta de Yanque: ejes y niveles primero). Dogfooding: pedirle el `.igz` con sus láminas. Después siguen en cola: atlas de escenas, metrados por material/capa.

## 📦 Estado anterior (2026-09-04) — v0.3.10 released: release urgente (Windows no arrancaba, visor en blanco con face-me de malla) + componentes compartidos, pulgadas, openskp 1.2.0
...
- **El formato por defecto es un punto único de fallo.** `setDefaultFormat(3.3 core)` no lo lee sólo el visor: lo lee el *backing store* de Qt. En una máquina cuyo EGL no tiene config de OpenGL de escritorio (NVIDIA + Wayland, reportado 09-09) no falla el visor — no abre la ventana, y el log es `Failed to create context: 3009` (`EGL_BAD_MATCH`) seguido de `Failed to create QRhi for QBackingStoreRhiSupport`. **`QT_QPA_PLATFORM="wayland;xcb"` NO rescata esto**: esa lista sólo se recorre al cargar el *plugin*, y el de Wayland carga bien; el fallo llega un paso después. Lo cubre `core/gl_fallback.py` (formato más flaco → reejecución bajo xcb). Y si el widget se fabrica su formato a mano, **que lo copie del de por defecto**, o deshace el rescate.
