...
**Los 67 que faltan (todos multi-step profundos, 3-7 ops):** estados con múltiples stacks Ctrl sobre caras inclinadas, la familia −50 sobre estados compuestos (incl. el degenerado "Ctrl-flush a través del sólido entero" que convierte un hueco en túnel — cube 190), y secuencias planta/grupo largas. Cada seed requiere entender 4+ pasos de estado; varios son semánticas degeneradas a decidir (¿qué *debe* hacer un Ctrl-stack de espacio cero?). Criterio invariable: minimizar seed → regresión nombrada → fix de raíz, nunca parche.
