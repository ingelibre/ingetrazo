...
✅ **Grieta + costuras del push anidado 3D (resuelto).** El bug de `pris2.igz` (push repetido sobre una pared-de-bump de un prisma irregular) tenía dos síntomas: (1) **cara faltante** → malla no-hermética; (2) **diagonales visibles** = caras coplanares sin fusionar (vs caras limpias de SketchUp). Ambos se cierran con **`cap_boundary_loops`** (`core/topology.py`), tras un push **sobre un sólido pre-existente** (`attached and was_solid`): traza cada loop cerrado de aristas que bordean **una sola cara** (una grieta) y lo rellena con una cara plana, y después **fusiona cada parche con su componente coplanar completo** (`dissolve_coplanar_region`, tolerante a winding) para que no queden costuras-diagonal. Gateado a sólidos — una hoja plana (recess sobre superficie) deja bordes abiertos legítimos, así que ahí no corre. Validado: las **60 combinaciones** de 3 push laterales (cualquier pared/orden) sobre un triángulo irregular dan malla **hermética, planar y sin costuras coplanares**. (El intento de reconstruir el plano entero con el arrangement creaba caras-fantasma del lado de afuera — "¿de qué lado está el sólido?"; tapar grietas por loop lo evita porque solo rellena lo ya rodeado de caras.) El arrangement plano sigue como **Edit ▸ Rebuild Faces (Planar)** para plantas a mano.

> ⚠️ **Honestidad arquitectónica:** lo de arriba (3D) son **parches/heals, no fix de raíz.** El `cap_boundary_loops` es un heal post-hoc (deja que `_extrude_commands` genere la grieta y *después* la rellena); el merge `abs()` tolerante a winding **tapa** que la malla no tiene orientación global consistente (se intentó voltear la base al extruir → rompió tests → revertido). La causa raíz —el árbol de casos frágil de `_extrude_commands` + winding inconsistente— sigue ahí. Robustos para lo validado, pero deuda arquitectónica.

### 🎯 PRÓXIMA SESIÓN — fix de raíz del motor (no más parches) — definida 2026-06-09

Reemplazar los heals 3D por las **dos** soluciones de raíz. Es trabajo de motor, sesión dedicada:
...
1. ✅ **Orientación consistente en la malla no-manifold — paso 1 + integración (hecho 2026-06-09).** `core/orient.py`: `orient_outward(mesh)` voltea las caras (in-place, preservando identidad de objeto) para que toda normal apunte hacia afuera del sólido. Como la malla es no-manifold (winding no se propaga por half-edge: una arista la comparten 2 muros + un piso), el "afuera" se decide **por cara, independiente**, vía **ray-casting de paridad** (un punto justo afuera de una cara está fuera del sólido si un rayo cruza el resto de la malla un nº par de veces; jitter multi-rayo con voto para esquivar grazes de aristas compartidas). Sin seed, robusto a no-manifold. **Gateado a mallas cerradas** (`is_closed`) → no-op en hojas planas/abiertas (la base free-standing se conserva). `signed_volume(mesh)` para confirmar consistencia. **Integrado en `pushpull._mutate`**: tras extrude + stitch + `cap_boundary_loops`, una pasada `orient_outward` deja todo sólido commiteado con winding consistente → **mata la clase "no puedo empujar esta cara / empuja hacia adentro"** (un strip fresco wound al revés, una tapa volteada, o la base del primer extrude flat→solid). 31 tests nuevos (`tests/test_orient.py` 11 + `tests/test_pushpull_orient.py` 20: las 60 combinaciones de push laterales hechas permanentes — cada secuencia commitea sólido cerrado, sin costuras, volumen+ y ya outward-consistente). 229 verdes.
   ...
   ⚠️ **Lo que NO se logró (y por qué):** quitar el `abs()` del merge coplanar. Probado empíricamente → rompe 11 tests. Causa: el merge de `run_stitch` corre sobre la malla **abierta** (el `cap_boundary_loops` cierra *después*), con fragmentos frescos de winding arbitrario, así que ahí el `abs()` es load-bearing. Y aún reordenando a cap-antes-de-merge, los casos legítimamente abiertos (recess sobre hoja plana, que `orient` no puede orientar) seguirían necesitándolo. **La eliminación real del `abs()` exige que `_extrude_commands` emita winding consistente desde el vamos** — el intento previo (voltear la base al extruir) rompió `test_standalone_face_keeps_base` y se revirtió. Ese es el verdadero núcleo restante del paso 1 (winding consistente en el extrude, no como heal post-hoc). El invariante de orientación al commit ya está; el `abs()` quedó como mecanismo interno honesto del merge sobre geometría cruda, no como crutch que tapa estado commiteado inconsistente.

2. ✅ **Integrar el planar arrangement al push/pull 3D — núcleo + crack-heal reemplazado (hecho 2026-06-09).** El bloqueante "¿de qué lado está el sólido?" **resuelto** con la orientación del punto 1: `core/cap_rebuild.py::rebuild_plane(mesh, origin, normal)` recompute un plano desde sus aristas — corre el `planar_arrangement`, clasifica cada región **inside/outside** por el **winding-number** del punto interior contra las **aristas de muro orientadas outward** (el muro define el "adentro"), descarta los fantasmas de afuera, y unifica las regiones sólidas (disolviendo aristas interiores) en caras finales con huecos. Validado: prisma irregular en todo orden de push (ambos caps), L cóncava con fantasma en el notch, caps con hueco. **Integrado** en `pushpull._mutate` (`apply_rebuild` + `crack_planes`): reemplaza `cap_boundary_loops` (el heal post-hoc de grietas) por el rebuild determinista del plano agrietado — llena la grieta **y** fusiona costuras en una pasada, sin árbol de casos. 30 tests nuevos (`tests/test_cap_rebuild.py` + round-trip). De paso, fix de un crash **pre-existente** de redo-tras-push (`SnapshotMutation` re-ejecutaba el closure con `base_face=None`; ahora restaura el resultado capturado como `SnapshotCompound`). 245 verdes.

   ⚠️ **Lo que queda (el rewrite grande):** reemplazar el **árbol de casos de `_extrude_commands`** (extend_wall_edge, strip, swept, step-notch) **y el merge `abs()` de `run_stitch` fase 3**. Se probó el drop-in (rebuild de *todos* los planos tocados sin el merge, `run_stitch(coplanar_merge=False)`) → **rompe 43 tests**: el rebuild del cap no cubre lo que hacen el extend de muros, el notch de gradas ni el recess en planos de pared. El `cap_rebuild` está listo y reemplaza limpio el crack-heal; reemplazar el extrude entero requiere que `cap_rebuild` (o un equivalente por-plano) cubra también esos casos de pared/grada, o reescribir `_extrude_commands` a "build naive + rebuild todos los planos". Sesión dedicada. El `run_stitch` ya acepta `coplanar_merge=False` (listo para cuando el rebuild cubra los planos de pared).

**Orden sugerido:** primero (1) orientación consistente — destrabó (2), porque "outward" resolvió "¿de qué lado está el sólido?". Conservar las 245 pruebas verdes + las 60 combinaciones de push como red de regresión. Banco de pruebas: `pris2.igz` y el triángulo irregular `[(-0.2,-2.8),(2.7,-7.2),(4.1,-2.7)]`.
