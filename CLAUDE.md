...
>
> 1. **4 seeds `KNOWN_BAD` restantes (todos draw-side):** `cube 121`, `plan 152`, `plan 210` (orphan edges al dibujar un rect), `plan 242` (seam coplanar). **NO son push/pull** — son del path de draw/heal (`edits.py` / `topology.py::heal_*`): `_draw_rect`/`build_add_edges` deja aristas huérfanas o una costura sin fusionar cuando un rectángulo aterriza sobre ciertas caras. Método: `run_sequence(escenario, seed)` para repro → minimizar → regresión → fix → `python -m tests.test_fuzz_engine` para regenerar.
