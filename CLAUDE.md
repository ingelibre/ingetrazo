...
1. **Grosores y relleno de corte en el estilo vectorial** (la diferencia entre «dibujo CAD» y «plano»): `hlr_view` funde las cuerdas de corte con las aristas duras (`core/hlr.py` `out_hard.extend(cut_edges)`) y `paint_frame_mm` usa UNA pluma de 0,22 → devolver clase por segmento (corte / silueta-perfil / arista), plumas 0,5 / 0,35 / 0,18 configurables en el marco, y polígonos de corte rellenos (sólido o achurado 45°) reutilizando la geometría del section fill del visor. Medio.
