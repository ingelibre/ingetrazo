...
1. ✅ **Orientación consistente en la malla no-manifold (hecho 2026-06-09).** `core/orient.py`: `orient_outward(mesh)` voltea las caras para que toda normal apunte hacia afuera del sólido. Como la malla es no-manifold (winding no se propaga por half-edge: una arista la comparten 2 muros + un piso), el "afuera" se decide **por cara, independiente**, vía **ray-casting de paridad** (un punto justo afuera de una cara está fuera del sólido si un rayo cruza el resto de la malla un nº par de veces; jitter multi-rayo con voto para esquivar grazes de aristas compartidas). Sin seed, robusto a no-manifold. **Gateado a mallas cerradas** (`is_closed`) → no-op en hojas planas/abiertas, lo que evita el bug que rompía `test_standalone_face_keeps_base` (la base free-standing se conserva). `signed_volume(mesh)` para confirmar consistencia. 11 tests en `tests/test_orient.py`; validado en geometría del push/pull **real** (prisma triangular irregular del banco + bump lateral con fragmentos coplanares → 0 caras no-outward, cerrado, volumen positivo). Falta el **paso 2** (integrarlo): llamar `orient_outward` en `pushpull._mutate` tras el extrude, quitar el `abs()` de `dissolve_coplanar_region`/`dissolve_edge` (volver a winding estricto), evaluar retirar `cap_boundary_loops`, y re-validar las 60 combinaciones de push.

2. **Integrar el planar arrangement al push/pull 3D** — recomputar la topología del plano afectado en vez de parchear con casos. El bloqueante real: **"¿de qué lado está el sólido?"** Reconstruir un plano desde *todas* sus aristas traza caras a ambos lados → caras-fantasma del lado de afuera. Hay que **clasificar interior/exterior del cap** (qué región bounded del arrangement es parte del sólido). Pistas: usar las paredes (cada arista de perímetro del cap bordea 1 cara-cap + 1 muro → el muro define el "adentro"), o el winding consistente del punto 1 (que da el lado outward directo). Si se logra, reemplaza `_extrude_commands` + `cap_boundary_loops` + el merge `abs()` de una.

**Orden sugerido:** primero (1) orientación consistente — destraba (2), porque "outward" resuelve "¿de qué lado está el sólido?". Conservar las 198 pruebas verdes + las 60 combinaciones de push como red de regresión. Banco de pruebas: `pris2.igz` y el triángulo irregular `[(-0.2,-2.8),(2.7,-7.2),(4.1,-2.7)]`.
