...
def test_move_drags_all_coincident_points():
    ...
    scene = Scene()
    hist = History(scene)
    left = scene.mesh.add_face([V(0, 0), V(1, 0), V(1, 1), V(0, 1)])
    right = scene.mesh.add_face([V(1, 0), V(2, 0), V(2, 1), V(1, 1)])
...
def test_ridge_move_deforms_roof_slopes():
    ...
    top = scene.mesh.add_face([V(0, 0, 2), V(4, 0, 2), V(4, 2, 2), V(0, 2, 2)])
...
def test_ridge_apex_propagates_into_gable_wall():
    ...
    scene.mesh.add_face(list(top))
    scene.mesh.add_face(list(wall))
